...
static int anansi_mq_set_partition_(MQ *self, const char *partition);
static const char *anansi_mq_partition_(MQ *self);
...
static MQCONNIMPL anansi_mq_connection_impl_ = {
	...
	anansi_mq_cluster_,
	anansi_mq_set_partition_,
	anansi_mq_partition_
};
...
anansi_mq_connect_recv_(MQ *self)
{
	...
	dburi = NULL;
/*	fprintf(stderr, PLUGIN_NAME ": MQ: establishing SQL connection to <%s>\n", self->uri + 7); */
	self->sql = sql_connect(self->uri + 7);
	if(!self->sql)
	{
		SET_ERRNO(self);
...
anansi_mq_next_(MQ *self, MQMESSAGE **msg)
{
	...
	*msg = NULL;
	...
	while(1)
	{
		...
		if(!rs)
		{
			fprintf(stderr,  PLUGIN_NAME ": MQ: %s\n", sql_error(self->sql));
			return -1;
		}
		if(sql_stmt_eof(rs))
		{
			BACKOFF_SECS(self, 1);
			SET_SYSERR(self, EAGAIN);
			sql_stmt_destroy(rs);
			return -1;
		}
		...
		{
			SET_ERRNO(self);
			fprintf(stderr, PLUGIN_NAME ": MQ: failed to create new message\n");
			return -1;
		}
		p->kind = MQK_INCOMING;
		/* Apply prefix */
		...
		{
			SET_ERRNO(self);
			fprintf(stderr, PLUGIN_NAME ": MQ: failed to duplicate buffer for incoming message\n");
			free(p);
			return -1;
		}
		strcpy(p->buf, "anansi:///");
...
/* Set the partition (if any) associated with this connection */
static int 
anansi_mq_set_partition_(MQ *self, const char *partition)
{
	char *p;
...
/* Return the partition (if any) associated with this connection */
static const char *
anansi_mq_partition_(MQ *self)
{
